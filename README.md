...
## 8-Week Cohort Curriculum — Version 6.0 (Final)
...
## Week 4 — Concurrency & System Design Synthesis (The AI Week)
**Theme:** The Bridge  
...
Participants choose **ONE** track. This is where Bitcoin depth is finally allowed to become primary.

1. **Mini Bitcoin Core:** Blockchain, validation, PoW, difficulty (ties directly back to the book's Ch. 5–7).
...
# Appendix: The Report Card

## Coverage of *Building Bitcoin in Rust*

| Book Part | Cohort Coverage | Notes |
|---|---|---|
| Prelude & Setup | 100% | Toolchain, Cargo, workspaces covered in onboarding + Week 7. |
| Ch. 1: Introduction | 80% | Rust/Bitcoin synergy discussed; genesis block concept used as intro example. |
| Ch. 2: Whitepaper | 60% | Transactions, blocks, PoW, network, incentives covered conceptually. Mining economics, halving, supply schedule omitted intentionally. |
| Ch. 3: Setting Up | 100% | Rustup, Cargo, crates, workspaces covered. |
| Ch. 4: Taming Rust | 95% | Ownership, borrowing, lifetimes, strings, pattern matching, structs, enums, traits, generics, closures, iterators all covered. |
| Ch. 5: Keeping Rust Cultured | 100% | `rustfmt`, `clippy`, error design (`thiserror`) integrated. |
| Ch. 6: Bitcoin Library | 55% | Data types, error handling, serialization, hashing concepts covered. Actual blockchain implementation, difficulty retargeting, and mining omitted. |
| Ch. 7: CPU Miner | 25% | PoW and nonce concepts touched in Week 8. Actual SHA-256 mining and difficulty adjustment omitted (capstone material). |
| Ch. 8: Bitcoin Node | 120% | Peer discovery, networking, sync, message handling covered more deeply via distributed systems theory. |
| Ch. 9: CLI/TUI Wallet | 30% | Wallet state modeled in Week 1. CLI/TUI frameworks omitted (not core to systems programming). |
| Ch. 10: Conclusion | N/A | Cohort provides its own capstone pathway. |

**Net Book Value:** ~75–80% of the book’s educational value, with the cohort going *deeper* on networking, async, and distributed systems while intentionally deferring full blockchain implementation to the capstone.

## Distributed Systems & Systems Engineering Coverage

| Concept | Cohort Week | Depth |
|---|---|---|
| State Machines | 1 | Deep |
| Type-Safe APIs / Invalid States | 2 | Deep |
| Indexing, Caching, Data Locality | 3 | Deep |
| Parallel Processing, Worker Pools | 4 | Deep |
| Protocol Design, Message Framing | 5 | Deep |
| Async I/O, Backpressure, Event Loops | 6 | Deep |
| Scheduling, Retry, Load Balancing | 7 | Medium |
| Gossip, DHTs, Peer Discovery | 8 | Deep |
| Consensus (Raft, Paxos, Nakamoto) | 8 | Conceptual |
| CAP Theorem, Failure Detection | 8 | Conceptual |

**Net Systems Value:** ~300% of the book’s distributed systems content. The cohort treats Bitcoin as *one instance* of P2P design, not the only one.
